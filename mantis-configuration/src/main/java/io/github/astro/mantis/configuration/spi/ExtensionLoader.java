...
import static io.github.astro.mantis.common.constant.Constant.SPI_FIX_PATH;
...
public class ExtensionLoader<S> {
    ...
    private void loadServiceClasses(Class<S> clazz) {
        ...
        try {
            ...
            if (!resources.hasMoreElements()) {
                throw new IllegalArgumentException("File: " + clazz.getTypeName() + " is not Exist,Current Directory: " + PREFIX);
            }
                    ...
                    while ((serviceClassName = br.readLine()) != null) {
                        try {
                            ...
                            if (serviceClass.isAnnotationPresent(ServiceProvider.class)) {
                                ...
                                String key = StringUtils.isBlankOrDefault(provider.value(),serviceClass.getName());
    ...
    public S getService(String serviceName) {
        ...
        if (service == null) {
            ...
            if (clazz == null) {
                throw new IllegalArgumentException("Load service type(" + this.type + ":" + serviceName + ") failed,unknown the " + serviceName + " Please check is Exist");
            }
    ...
    public S getDefault() {
        if (!StringUtils.isBlank(defaultService)) {
            if (services.size() > 1) {
    ...
    private S createInstance(Class<? extends S> clazz) {
        S service = ReflectUtils.createInstance(clazz);
